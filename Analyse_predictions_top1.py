...
ici on regarde le prediction du top 1 pour comparer avec l'article HOBBIT
"""
import torch
import sys
...
def calcul_hit(k=step_prediction):
    hit = 0
    nb_total = 0
    data = torch.load(f"prompt_instructions/prediction_realite_k={k}.pt")  # chemin 
    #recuperer les dico
    prediction = data["prediction"]  # tensor
    realite = data["realite"]

    # Pour chaque couche
    for layer in prediction:
        for token in range(len(prediction[layer])):
            expert = prediction[layer][token][0] #[0] = top-1
                #print('couche : ', layer)
                #print('token : ', token)
                #print("prediction : ", prediction[layer][token][expert], "realite : ", realite[layer][token])
                # Si l'expert est le même que dans la réalité
            if expert in realite[layer][token]: 
                #on regarde si le top 1 de la prediction est dans les top2 de la realite
                hit += 1
                    #print("hit")
            nb_total += 1
